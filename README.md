...
[![CI](https://github.com/shrutilekkala/fastapi-ai-telemetry/actions/workflows/ci.yml/badge.svg)](https://github.com/shrutilekkala/fastapi-ai-telemetry/actions/workflows/ci.yml)
...
The test suite exercises the behavior that matters at the instrumentation boundary:

- request IDs supplied by clients and generated by the middleware
- concurrent request isolation through `ContextVar`
- model-call timing, token aggregation, and failure recording
- sensitive-header redaction and explicit header allowlists
- exporter failure isolation
- bounded Prometheus labels and metric aggregation

CI runs Ruff and pytest on Python 3.10 through 3.13, then builds the container image in a separate job.
